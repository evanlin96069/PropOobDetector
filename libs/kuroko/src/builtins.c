...
KRK_Function(print) {
	...
	for (int i = 0; i < remArgc; ++i) {
		...
		if (!IS_NONE(file)) {
			...
			krk_fwrite(AS_CSTRING(printable), AS_STRING(printable)->length, 1, stdout);
			krk_pop();
			if (i + 1 < remArgc) krk_fwrite(sep, sepLen, 1, stdout);
		}
	}
	...
	if (!IS_NONE(file)) {
		...
		krk_fwrite(end, endLen, 1, stdout);
		if (flush) krk_fflush(stdout);
	}
