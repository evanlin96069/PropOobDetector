...
void * krk_reallocate(void * ptr, size_t old, size_t new) {
	...
	if (ptr) {
		if (!_debug_mem_has(ptr)) {
			krk_fprintf(stderr, "Invalid reallocation of %p from %zu to %zu\n", ptr, old, new);
			abort();
		}

		size_t t = _debug_mem_get(ptr);
		if (t != old) {
			krk_fprintf(stderr, "Invalid reallocation of %p from %zu - should be %zu - to %zu\n", ptr, old, t, new);
			abort();
		}
...
size_t krk_collectGarbage(void) {
	...
	if (vm.globalFlags & KRK_GLOBAL_REPORT_GC_COLLECTS) {
		...
		krk_fprintf(stderr, "[gc] %lld.%.9lds %s before; %s after; freed %s in %llu objects; next collection at %s\n",
