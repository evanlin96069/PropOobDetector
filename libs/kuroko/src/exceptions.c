...
static void dumpInnerException(KrkValue exception, int depth) {
	if (depth > 10) {
		krk_fprintf(stderr, "Too many inner exceptions encountered.\n");
		return;
	}
	...
	if (IS_INSTANCE(exception)) {
		...
		if (krk_tableGet(&AS_INSTANCE(exception)->fields, OBJECT_VAL(S("traceback")), &tracebackEntries)
			...
			krk_fprintf(stderr, "Traceback (most recent call last):\n");
			for (size_t i = 0; i < AS_LIST(tracebackEntries)->count; ++i) {
				...
				krk_fprintf(stderr, "  File \"%s\", line %d, in %s\n",
					...
					do {
						...
						if (line == lineNo) {
							...
							if (krk_debug_expressionUnderline(function, &start, &midStart, &midEnd, &end, instruction)) {
								krk_fprintf(stderr,"    ");
								for (; j < start; ++j) krk_fprintf(stderr," ");
								for (; j < midStart; ++j) krk_fprintf(stderr,"~");
								for (; j < midEnd; ++j) krk_fprintf(stderr, "^");
								for (; j < end; ++j) krk_fprintf(stderr,"~");
								krk_fprintf(stderr,"\n");
							}
#endif
							break;
	...
	if (!(IS_NONE(module) || (IS_STRING(module) && AS_STRING(module) == S("builtins")))) {
		krk_fprintf(stderr, "%s.", AS_CSTRING(module));
	}

	/* Print type name */
	krk_fprintf(stderr, "%s", krk_typeName(exception));
	...
	if (!IS_STRING(result) || AS_STRING(result)->length == 0) {
		...
		krk_fprintf(stderr, ": ");
		krk_fwrite(AS_CSTRING(result), AS_STRING(result)->length, 1, stderr);
		krk_fprintf(stderr, "\n");
	}
