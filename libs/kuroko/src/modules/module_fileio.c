...
KRK_Method(File,write) {
	...
	return INTEGER_VAL(krk_fwrite(AS_CSTRING(argv[1]), 1, AS_STRING(argv[1])->length, file));
}
...
KRK_Method(BinaryFile,write) {
	...
	return INTEGER_VAL(krk_fwrite(AS_BYTES(argv[1])->bytes, 1, AS_BYTES(argv[1])->length, file));
}
