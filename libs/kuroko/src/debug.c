...
void krk_printValueSafe(FILE * f, KrkValue printable) {
	if (!IS_OBJECT(printable)) {
		switch (KRK_VAL_TYPE(printable)) {
			case KRK_VAL_INTEGER:  krk_fprintf(f, PRIkrk_int, AS_INTEGER(printable)); break;
			case KRK_VAL_BOOLEAN:  krk_fprintf(f, "%s", AS_BOOLEAN(printable) ? "True" : "False"); break;
			case KRK_VAL_NONE:     krk_fprintf(f, "None"); break;
			case KRK_VAL_HANDLER:
				switch (AS_HANDLER_TYPE(printable)) {
					case OP_PUSH_TRY:      krk_fprintf(f, "{try->%d}",     (int)AS_HANDLER_TARGET(printable)); break;
					case OP_PUSH_WITH:     krk_fprintf(f, "{with->%d}",    (int)AS_HANDLER_TARGET(printable)); break;
					case OP_RAISE:         krk_fprintf(f, "{raise<-%d}",   (int)AS_HANDLER_TARGET(printable)); break;
					case OP_FILTER_EXCEPT: krk_fprintf(f, "{except<-%d}",  (int)AS_HANDLER_TARGET(printable)); break;
					case OP_BEGIN_FINALLY: krk_fprintf(f, "{finally<-%d}", (int)AS_HANDLER_TARGET(printable)); break;
					case OP_RETURN:        krk_fprintf(f, "{return<-%d}",  (int)AS_HANDLER_TARGET(printable)); break;
					case OP_END_FINALLY:   krk_fprintf(f, "{end<-%d}",     (int)AS_HANDLER_TARGET(printable)); break;
					case OP_EXIT_LOOP:     krk_fprintf(f, "{exit<-%d}",    (int)AS_HANDLER_TARGET(printable)); break;
					case OP_RAISE_FROM:    krk_fprintf(f, "{reraise<-%d}", (int)AS_HANDLER_TARGET(printable)); break;
				}
				break;
			case KRK_VAL_KWARGS: {
				if (AS_INTEGER(printable) == KWARGS_SINGLE) {
					krk_fprintf(f, "{unpack single}");
				} else if (AS_INTEGER(printable) == KWARGS_LIST) {
					krk_fprintf(f, "{unpack list}");
				} else if (AS_INTEGER(printable) == KWARGS_DICT) {
					krk_fprintf(f, "{unpack dict}");
				} else if (AS_INTEGER(printable) == KWARGS_NIL) {
					krk_fprintf(f, "{unpack nil}");
				} else if (AS_INTEGER(printable) == KWARGS_UNSET) {
					krk_fprintf(f, "{unset default}");
				} else {
					krk_fprintf(f, "{sentinel=" PRIkrk_int "}",AS_INTEGER(printable));
				}
				break;
			}
			default:
#ifndef KRK_NO_FLOAT
				if (IS_FLOATING(printable)) krk_fprintf(f, "%.16g", AS_FLOATING(printable));
#endif
				break;
		}
		...
		for (size_t c = 0; c < AS_STRING(printable)->length && c < STRING_DEBUG_TRUNCATE; ++c) {
			...
			switch (byte) {
				case '\\': krk_fprintf(f, "\\\\"); break;
				case '\n': krk_fprintf(f, "\\n"); break;
				case '\r': krk_fprintf(f, "\\r"); break;
				case '\'': krk_fprintf(f, "\\'"); break;
				default: {
					if (byte < ' ' || byte > '~') {
						krk_fprintf(f, "\\x%02x", byte);
					} else {
						krk_fprintf(f, "%c", byte);
					}
					break;
				}
			}
		...
		switch (AS_OBJECT(printable)->type) {
			case KRK_OBJ_CODEOBJECT: krk_fprintf(f, "<codeobject %s>", AS_codeobject(printable)->name ? AS_codeobject(printable)->name->chars : "?"); break;
			case KRK_OBJ_CLASS: krk_fprintf(f, "<class %s>", AS_CLASS(printable)->name ? AS_CLASS(printable)->name->chars : "?"); break;
			case KRK_OBJ_INSTANCE: krk_fprintf(f, "<instance of %s>", AS_INSTANCE(printable)->_class->name->chars); break;
			case KRK_OBJ_NATIVE: krk_fprintf(f, "<nativefn %s>", ((KrkNative*)AS_OBJECT(printable))->name); break;
			case KRK_OBJ_CLOSURE: krk_fprintf(f, "<function %s>", AS_CLOSURE(printable)->function->name->chars); break;
			case KRK_OBJ_BYTES: krk_fprintf(f, "<bytes of len %ld>", (long)AS_BYTES(printable)->length); break;
			case KRK_OBJ_TUPLE: {
				krk_fprintf(f, "(");
			...
			case KRK_OBJ_BOUND_METHOD: krk_fprintf(f, "<method %s>",
			...
			default: krk_fprintf(f, "<%s>", krk_typeName(printable)); break;
		}
	}
}
...
void krk_debug_dumpStack(FILE * file, KrkCallFrame * frame) {
	...
	for (KrkValue * slot = krk_currentThread.stack; slot < krk_currentThread.stackTop; slot++) {
		krk_fprintf(file, "[%c", frame->slots == i ? '*' : ' ');
		...
		for (size_t x = krk_currentThread.frameCount; x > 0; x--) {
			...
			for (size_t j = 0; j < f->closure->function->localNameCount; ++j) {
				if (relative == f->closure->function->localNames[j].id
					...
					krk_fprintf(file, "%s=", f->closure->function->localNames[j].name->chars);
					found = 1;
					break;
				}
			}
...
void krk_disassembleCodeObject(FILE * f, KrkCodeObject * func, const char * name) {
	...
	/* Function header */
	krk_fprintf(f, "<%s(", name);
	int j = 0;
	for (int i = 0; i < func->potentialPositionals; ++i) {
		krk_fprintf(f,"%s",func->localNames[j].name->chars);
		if (j + 1 < func->totalArguments) krk_fprintf(f,",");
		j++;
	}
...
static void _print_opcode(OPARGS) {
	krk_fprintf(f, "%-16s ", opcodeClean(fullName));
}
...
static void _jump(OPARGS, int sign) {
	...
	krk_fprintf(f, "%4d (to %d)", (int)jump, (int)(*offset + 3 + sign * jump));
	*size = 3;
}
...
#define SIMPLE(opc)
#define JUMP(opc,sign) case opc: krk_fprintf(f, "(%s, to %zu)", opcodeClean(#opc), *offset + 3 sign current_jump); return;
...
#include "opcodes.h"
	...
	krk_fprintf(f,"(invalid destination)");
}
#undef SIMPLE
#undef OPERAND
#undef CONSTANT
		...
		if (isLocal & 1) {
			for (size_t i = 0; i < func->localNameCount; ++i) {
				if (func->localNames[i].id == (size_t)index && func->localNames[i].birthday <= *offset && func->localNames[i].deathday >= *offset) {
					krk_fprintf(f, "%s", func->localNames[i].name->chars);
					break;
				}
			}
		} else if (isLocal & 4) {
			krk_fprintf(f, "classcell");
		} else { krk_fprintf(f, "upvalue<%d>", index); }
		if (j + 1 != function->upvalueCount) krk_fprintf(f, ", ");
	}
}
...
static void _expand_args_more(OPARGS, size_t operand) {
	krk_fprintf(f, " (%s)", operand == 0 ? "singleton" : (operand == 1 ? "list" : "dict"));
}
...
static void _format_value_more(OPARGS, size_t operand) {
	if (operand != 0) {
		int hasThing = 0;
		krk_fprintf(f, " (");
		if (operand & FORMAT_OP_EQ)     { krk_fprintf(f, "eq"); hasThing = 1; }
		if (operand & FORMAT_OP_STR)    { krk_fprintf(f, "%sstr", hasThing ? ", " : ""); hasThing = 1; }
		if (operand & FORMAT_OP_REPR)   { krk_fprintf(f, "%srepr", hasThing ? ", " : ""); hasThing = 1; }
		if (operand & FORMAT_OP_FORMAT) { krk_fprintf(f, "%swith format", hasThing ? ", " : ""); }
		krk_fprintf(f, ")");
	}
}
...
size_t krk_disassembleInstruction(FILE * f, KrkCodeObject * func, size_t offset) {
	...
	if (offset > 0 && krk_lineNumber(chunk, offset) == krk_lineNumber(chunk, offset - 1)) {
		...
		if (offset > 0) krk_fprintf(f,"\n");
		krk_fprintf(f, "%4d ", (int)krk_lineNumber(chunk, offset));
	}
	...
	krk_fprintf(f, "%4u ", (unsigned int)offset);
	uint8_t opcode = chunk->code[offset];
	size_t size = 1;
...
#include "opcodes.h"
		default:
			krk_fprintf(f, "Unknown opcode: %02x", opcode);
	}
